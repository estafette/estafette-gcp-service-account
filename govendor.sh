govendor init
govendor fetch github.com/ericchiang/k8s/...@v1.2.0
govendor fetch github.com/ericchiang/k8s/apis/extensions/v1beta1/...@v1.2.0
govendor fetch github.com/prometheus/client_golang/prometheus/...@v0.9.2
govendor fetch github.com/rs/zerolog/...@v1.11.0
govendor fetch github.com/stretchr/testify/assert@v1.3.0
govendor fetch github.com/alecthomas/kingpin@v2.2.5
govendor fetch golang.org/x/oauth2/google
govendor fetch google.golang.org/api/iam/v1